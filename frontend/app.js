...
const Topics = () => {
  ...
  const handleReset = async (e, topic) => {
    e.stopPropagation();
    if (confirm(`Are you sure you want to reset all progress for "${topic.name}"? This will clear your mastery and learned stats.`)) {
      try {
  ...
  const handleDeleteTopic = async (e, topic) => {
    e.stopPropagation();
    if (confirm(`Are you sure you want to delete "${topic.name}"? This will permanently remove all cards and progress.`)) {
      try {
                    ...
                    >
                      ...
                      {isSelectionMode && (
                        <div className="absolute top-6 right-6 z-20">
                          <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-primary border-primary' : 'border-gray-600'}`}>
                            {isSelected && <Icon name="check-circle" className="w-4 h-4 text-white" />}
                          </div>
                        </div>
                      )}
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
               ...
               <div className="space-y-4">
                  {Array.isArray(data) && data?.filter(t => t.parent_id === activeSyllabusId).map((topic) => {
                     ...
                     return (
                       <div key={topic.id} 
                         onClick={() => isSelectionMode ? toggleSelect(topic.topic_id) : null}
                         className={`bg-surface border rounded-2xl p-5 relative transition-all ${isSelectionMode ? 'cursor-pointer' : ''} ${isSelected ? 'border-primary ring-1 ring-primary/50 bg-primary/5' : 'border-gray-800'}`}
                       >
                          {isSelectionMode && (
                            <div className="absolute top-4 right-4">
                              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-primary border-primary' : 'border-gray-600'}`}>
                                {isSelected && <Icon name="check-circle" className="w-3 h-3 text-white" />}
                              </div>
                            </div>
                          )}
                          ...
                          <div className="flex gap-2">
                             {!isSelectionMode ? (
                               <>
                                 <button onClick={() => navigate(`/learn/${topic.topic_id}`)} className="flex-1 text-xs font-bold text-white bg-primary py-2.5 rounded-xl hover:bg-primaryFocus transition-colors">Study</button>
                                 <button onClick={() => navigate(`/quiz/${topic.topic_id}`)} className="flex-1 text-xs font-bold text-black bg-white py-2.5 rounded-xl hover:bg-gray-200 transition-colors">Quiz</button>
                                 <div className="flex gap-1">
                                   <button onClick={(e) => handleReset(e, topic)} title="Reset Progress" className="p-2.5 text-gray-500 hover:text-white bg-gray-900 rounded-xl hover:bg-gray-800 transition-all"><Icon name="refresh-cw" className="w-4 h-4" /></button>
                                   <button onClick={(e) => handleDeleteTopic(e, topic)} title="Delete Topic" className="p-2.5 text-gray-500 hover:text-rose-500 bg-gray-900 rounded-xl hover:bg-rose-500/10 transition-all"><Icon name="trash-2" className="w-4 h-4" /></button>
                                 </div>
                               </>
                             ) : (
                               <div className="flex-1 text-center py-2 text-xs font-bold text-gray-500 uppercase tracking-widest">
                                 {isSelected ? 'Selected for Action' : 'Click to Select'}
                               </div>
                             )}
                          </div>
                       </div>
